import {useState} from 'react';
import { Grid, TextField, Button, Box, Alert } from "@mui/material";


const SendPasswordResetEmail = () => {

  const [error, setError] = useState({
    status:false,
    msg:"",
    type: ""
  })

    const handleSubmit = (e) => {
        e.preventDefault();
        const data = new FormData(e.currentTarget);
        const actualData = {
          email: data.get('email'),
        }
        // console.log(actualData);
        if(actualData.email){
          console.log(actualData);
          document.getElementById('password-reset-form').reset();
          setError({status:true, msg: "Password Reset Email Send. Check Your Email!", type:"success"})
        } else{
          setError({status:true, msg: "Please Enter Your Valid Email ID", type:"error"})
        }
    }


  return (
    <>
       <Grid container justifyContent='center'>
         <Grid item sm={6} xs={12}>
         <Box component='form' noValidate sx={{mt:1}} id="password-reset-form" onSubmit={handleSubmit}>
        <TextField required margin="normal" fullWidth id='email' name='email' label='email' type='email' />
        <Box textAlign='center'>
        <Button type="submit" variant="contained" sx={{mt:3, mb:2, px:5}}>Send</Button>
        </Box>
        {error.status ? <Alert severity={error.type}>{error.msg}</Alert> : ""}
      </Box>
         </Grid>
       </Grid>
    </>
  )
}

export default SendPasswordResetEmail;
